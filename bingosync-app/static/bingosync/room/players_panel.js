var PlayersPanel = (function () {
  ...
  PlayersPanel.prototype.setPlayer = function (playerJson) {
    if (this.$playersPanel.find("#" + playerJson["uuid"]).length === 0) {
      ...
      var goalCounter = $("<span>", {
        class: "goalcounter " + colorClass,
        html: '<span class="squarecounter" title="Squares with color.">0</span> <span class="rowcounter" title="Rows with color.">(0)</span>',
      });

      var playerName = $("<span>", {
        class: "playername",
        text: " " + playerJson["name"],
      });
      ...
      this.$playersPanel.insertOnce(playerDiv, function ($possibleNext) {
        var possibleNextName = $.trim(
          $possibleNext.find(".playername").text(),
        ).toLowerCase();
  ...
  PlayersPanel.prototype.updateGoalCounters = function (board) {
    this.$playersPanel.find(".goalcounter").each(function () {
      ...
      $(this)
        .find(".rowcounter")
        .html("(" + board.getRowCount(colorClass) + ")");
    });
  };
