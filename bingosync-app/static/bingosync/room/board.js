var Board = (function () {
  ...
  function updateColorOffsets($square) {
    ...
    for (var i = 1; i < $colorElements.length; ++i) {
      var transform =
        "skew(-" + targetAngle + "rad) translateX(" + translations[i] + "%)";
  ...
  function setSquareColors($square, colors) {
    ...
    colors.forEach(function (color) {
      shadow.before(
        '<div class="bg-color ' + getSquareColorClass(color) + '"></div>',
      );
    });
  ...
  Square.prototype.setJson = function (json) {
    this.$square.html(
      '<div class="starred hidden"></div><div class="shadow"></div>' +
  ...
  var Board = function (
    ...
    var that = this;
    if (!this.isSpectator) {
      this.$squares.on("click", function (ev) {
        that.clickSquare(ev, $(this));
      });
    }
    ...
    $(window).resize(function () {
      that.$squares.each(function () {
        updateColorOffsets($(this));
      });
    });
