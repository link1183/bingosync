var ColorChooser = (function () {
  ...
  var ColorChooser = function ($chooserPanel, playerJson, colorSelectedUrl) {
    ...
    if (!this.isSpectator) {
      ...
      this.$choosers.on("click", function (ev) {
        that.clickColor(ev, $(this));
      });
    }
    this.$collapseButton.on("mousedown", function (ev) {
      that.toggleCollapse(ev);
    });
  };
  ...
  ColorChooser.prototype.setChosenColor = function (newColor) {
    ...
    var newColorClass =
      newColor.indexOf("square") !== -1
        ? newColor
        : getSquareColorClass(newColor);
