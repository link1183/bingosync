var ChatPanel = (function () {
  ...
  function processChatJson(json) {
    ...
    var timeString =
      zeroPad(eventTime.getHours(), 2) +
      ":" +
      zeroPad(eventTime.getMinutes(), 2) +
      ":" +
    ...
    if (json["type"] === "connection") {
      ...
      var connectionMessage = $("<span>", {
        text: connectionPlayerName + " " + json["event_type"],
      }).toHtml();
      return $("<div>", {
        class: "connection-message",
        html: timeHtml + " - " + connectionMessage,
      }).toHtml();
    } else if (json["type"] === "color") {
      var playerColorClass = getPlayerColorClass(json["player_color"]);
      var playerName = $("<span>", {
        class: playerColorClass,
        text: json["player"]["name"],
      }).toHtml();
      var newColorClass = getPlayerColorClass(json["color"]);
      var colorName = $("<span>", {
        class: "color-name " + newColorClass,
        text: json["color"],
      }).toHtml();
      var colorMessage = playerName + " changed color to " + colorName;
      return $("<div>", {
        class: "color-message",
        html: timeHtml + " " + colorMessage,
      }).toHtml();
      ...
      return $("<div>", {
        ...
        html: timeHtml + " " + newCardMessage,
      }).toHtml();
    }

    // otherwise first format the name of the message sender
    var playerSpan = processPlayerJson(
      json["player"],
      getPlayerColorClass(json["player_color"]),
    );
    if (json["type"] === "chat") {
      var message = $("<span>", {
        class: "chat-message",
      ...
      return $("<div>", {
        html: timeHtml + " " + playerSpan + ": " + message,
      }).toHtml();
    } else if (json["type"] === "goal") {
      var colorClass = json["remove"]
        ? getPlayerColorClass("blank")
        : getPlayerColorClass(json["color"]);
      var goal = $("<span>", {
        class: "goal-name " + colorClass,
        text: json["square"]["name"],
      }).toHtml();

      if (json["remove"]) {
        return $("<div>", {
          class: "goal-message",
          html: timeHtml + " " + playerSpan + " cleared " + goal,
        }).toHtml();
      } else {
        return $("<div>", {
          class: "goal-message",
          html: timeHtml + " " + playerSpan + " marked " + goal,
        }).toHtml();
      }
    }
  }
  ...
  ChatPanel.prototype.populateChatHistory = function (result) {
    ...
    if (!result.allIncluded) {
      var link = $("<div>", {
        class: "chat-link",
        html: "Click to load full history",
      });
      link.click(function () {
        that.reloadChatHistory(true);
      });
    ...
    for (var i = 0; i < result.events.length; i++) {
      ...
      var entry = $("<div>", {
        class: chatJson["type"] + "-entry",
        html: message,
      });
      this.$chatHistory.append(entry);
    }
    var $seedInChat = $("#bingo-chat .new-card-message .seed-wait")
      .removeClass("seed-wait")
      .addClass("seed-hidden")
      .text("Hidden");
    if (this.$chatBody[0] !== undefined) {
      this.$chatBody.scrollTop(this.$chatBody[0].scrollHeight);
    }
