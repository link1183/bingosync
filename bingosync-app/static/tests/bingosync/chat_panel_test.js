(function () {
  ...
  QUnit.test("Construct empty panel", function (assert) {
    ...
    assert.elementsAre(
      chatHistoryEl.children,
      assert.outerHtmlEquals.bind(assert),
  ...
  QUnit.test("Reload empty chat", function (assert) {
    ...
    var that = this;
    this.mockReloadChat(
      { events: [], allIncluded: true },
      assert.async(),
      function () {
        assert.elementsAre(
          that.chatBodyEl.children,
          assert.outerHtmlEquals.bind(assert),
          ['<div class="chat-history"></div>'],
        );
      },
  ...
  QUnit.test("Reload non-empty chat", function (assert) {
    ...
    var that = this;
    this.mockReloadChat(
      { events: events, allIncluded: true },
      assert.async(),
      function () {
        assert.elementsAre(
          that.chatBodyEl.children,
  ...
  QUnit.test("Reload partial chat history", function (assert) {
    ...
    chatPanel.populateChatHistory({
      events: initialEvents,
      allIncluded: false,
    });
    assert.elementsAre(
      this.chatBodyEl.children,
      assert.outerHtmlEquals.bind(assert),
      [
        '<div class="chat-history">' +
    ...
    var that = this;
    this.mockReloadFullChat(
      { events: allEvents, allIncluded: true },
      assert.async(),
      function () {
        assert.elementsAre(
          that.chatBodyEl.children,
