  ...
  var DOMAssertions = /** @class */ (function () {
    ...
    DOMAssertions.prototype.hasAttribute = function (name, value, message) {
      ...
      if (value instanceof RegExp) {
        ...
        var expected =
          "Element " + this.targetDescription + ' has attribute "' + name + '"';
        var actual = result
          ? expected
          : "Element " +
            this.targetDescription +
     ...
     * @see {@link #hasClass}
     */
    DOMAssertions.prototype.hasPseudoElementStyle = function (
      selector,
      expected,
      message,
    ) {
    ...
    DOMAssertions.prototype.hasText = function (expected, message) {
      ...
      if (expected instanceof RegExp) {
        ...
        var actual = result
          ? expected_1
          : "Element " + this.targetDescription + " has no text";
        if (!message) {
          message = expected_1;
        }
        this.pushResult({
        ...
        throw new TypeError(
          'You must pass a string or Regular Expression to "hasText". You passed ' +
            expected +
            ".",
        );
      }
    ...
    DOMAssertions.prototype.doesNotIncludeText = function (text, message) {
      ...
      if (!result) {
        actual =
          "Element " + this.targetDescription + ' includes text "' + text + '"';
      }
      if (!message) {
        message = expected;
      }
    ...
    DOMAssertions.prototype.doesNotContainText = function (
      unexpected,
      message,
    ) {
      this.doesNotIncludeText(unexpected, message);
    };
    DOMAssertions.prototype.doesNotHaveTextContaining = function (
      unexpected,
      message,
    ) {
      this.doesNotIncludeText(unexpected, message);
    ...
    DOMAssertions.prototype.hasValue = function (expected, message) {
      ...
      if (expected instanceof RegExp) {
        ...
        var actual = result
          ? expected_2
          : "Element " + this.targetDescription + " has no value";
        if (!message) {
          message = expected_2;
        }
        this.pushResult({
     ...
     */
    DOMAssertions.prototype.matchesSelector = function (
      compareSelector,
      message,
    ) {
      var targetElements =
        this.target instanceof Element ? [this.target] : this.findElements();
      ...
      if (matchFailures === 0) {
        ...
        actual = singleElement
          ? message
          : difference + " elements matched " + compareSelector + ".";
    ...
    DOMAssertions.prototype.doesNotMatchSelector = function (
      ...
      if (matchFailures === targets) {
        ...
        expected = singleElement
          ? message
          : targets +
            " elements should not have matched " +
            compareSelector +
            ".";
        this.pushResult({
          result: false,
    ...
    DOMAssertions.prototype.hasTagName = function (tagName, message) {
      ...
      if (typeof tagName !== "string") {
        throw new TypeError(
          'You must pass a string to "hasTagName". You passed ' + tagName + ".",
        );
      }
      ...
      if (actual === expected) {
        if (!message) {
          message =
            "Element " + this.targetDescription + " has tagName " + expected;
        }
        this.pushResult({
          result: true,
          actual: actual,
    ...
    DOMAssertions.prototype.doesNotHaveTagName = function (tagName, message) {
      ...
      if (typeof tagName !== "string") {
        throw new TypeError(
          'You must pass a string to "doesNotHaveTagName". You passed ' +
            tagName +
            ".",
        );
      }
    ...
    DOMAssertions.prototype.findTargetElement = function () {
      ...
      if (el === null) {
        var message =
          "Element " + (this.target || "<unknown>") + " should exist";
