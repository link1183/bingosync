...
(function (QUnit, window, undefined) {
  ...
  var getElementStyles = (function () {
    ...
    var camelCase = (function () {
      var camelCaseFn = (function () {
        ...
        return function (s) {
          return s
            .replace(rmsPrefix, msPrefixFix)
            .replace(rdashAlpha, camelCaseReplacerFn);
        };
      })();
  ...
  var serializeElementNode = function (elementNode, rootNodeStyles) {
    ...
    for (i = 0, len = subNodes.length; i < len; i++) {
      ...
      if (attrName === "class") {
        serializedNode.Attributes[attrName] = normalizeWhitespace(
          subNodes[i].value,
        );
    ...
    for (i = 0, len = subNodes.length; i < len; i++) {
      serializedNode.ChildNodes.push(
        serializeNode(subNodes[i], rootNodeStyles),
      );
    }
  ...
  var serializeNode = function (node, rootNodeStyles) {
    ...
    switch (node.nodeType) {
      ...
      case 2: // Node.ATTRIBUTE_NODE
        throw new Error(
          "`node.nodeType` was `Node.ATTRIBUTE_NODE` (2), which is not supported by this method",
        );
      default:
      ...
      QUnit.begin(function () {
        ...
        if (!iframe || !iframeWin || !iframeDoc) {
          ...
          iframeWin =
            ...
            (iframe.document &&
              (iframe.document.defaultView || iframe.document.window)) ||
            window.frames[iframe.name || iframe.id];

          iframeDoc =
            (iframeWin && iframeWin.document) ||
  ...
  QUnit.extend(QUnit.assert, {
    ...
    notHtmlEqual: function (actual, expected, message) {
      ...
      this.notDeepEqual(
        serializeHtml(actual),
        serializeHtml(expected),
        message,
      );
    },
