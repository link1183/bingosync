...
(function (root, factory) {
  ...
  function getMockForRequest(handler, requestSettings) {
    ...
    if ($.isFunction(handler.url.test)) {
      ...
      var namespace =
        handler.namespace ||
        (typeof handler.namespace === "undefined" &&
      ...
      if (
        (effecitveUrl !== requestSettings.url && star === -1) ||
        !new RegExp(
          effecitveUrl
            .replace(/[-[\]{}()+?.,\\^$|#\s]/g, "\\$&")
            .replace(/\*/g, ".+"),
        ...
        return function () {
          ...
          var finishRequest = function () {
            ...
            if ($.isArray(mockHandler.status)) {
              var idxStatus = Math.floor(
                Math.random() * mockHandler.status.length,
              );
  ...
  function handleAjax(url, origSettings) {
    ...
    for (var k = 0; k < mockHandlers.length; k++) {
      var handlerIndex = $.mockjaxSettings.matchInRegistrationOrder
        ? k
        : mockHandlers.length - 1 - k;
      ...
      if (
        ...
        (requestSettings.type.toUpperCase() === "GET" ||
          requestSettings.type.toUpperCase() === "HEAD") &&
