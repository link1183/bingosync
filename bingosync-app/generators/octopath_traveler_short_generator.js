...
bingoGenerator = function (bingoList, opts) {
  ...
  function magicSquare() {
    ...
    if (rf == 1) template.reverse();

    function inverse(t) {
      //inverts the table
      var s = [];
      for (var j = 0; j < t.length; j++) s.push([]);
      for (var j = 0; j < t.length; j++) {
        for (var k = 0; k < t.length; k++) s[j][k] = t[k][j];
      }
    }
  ...
  function preprocessBingoList(bingoList) {
    for (const key of Object.keys(bingoList)) {
      ...
      if (!bingoList[key].hasOwnProperty("Diff")) bingoList[key].Diff = 0;

      if (!bingoList[key].hasOwnProperty("Types")) bingoList[key].Types = [];
      ...
      if (!bingoList[key].hasOwnProperty("Score")) bingoList[key].Score = 0;
    }
  }
  ...
  var choosable = [];
  var unchoosable = [];
  for (const key of Object.keys(bingoList)) choosable.push(key);

  //Create counts for all types
  var types = {};
  for (const key of Object.keys(bingoTypes)) {
    if (!bingoTypes[key].hasOwnProperty("Max")) bingoTypes[key].Max = 5;
  ...
  var chosenGoals = [];
  for (var i = 1; i <= 25; i++) chosenGoals.push("");
  test_diff.push(0);
  ...
  for (var i = 1; i <= 25; i++) {
    ...
    for (var j = 0; j < goal.Synergy.length; j++) {
      var temp = [];
      for (var k = 0; k < choosable.length; k++) {
        if (goal.Synergy[j] == choosable[k])
          //check if the goal itself is a synergy
          temp.push(choosable[k]);
        for (var l = 0; l < bingoList[choosable[k]].Synergy.length; l++) {
          //check if it shares a synergy group that isn't an existing goal
          if (
            goal.Synergy[j] == bingoList[choosable[k]].Synergy[l] &&
