...
var NORMAL_PROFILE = {
  ...
  defaultMaximumIndividualSynergy:
...
var BingoGenerator = function (bingoList, options) {
  ...
  if (bingoList.info && bingoList.info.combined === "true") {
    if (bingoList[this.mode]) {
      ...
      console.log(
        "bingoList doesn't contain a valid sub goal list for mode: \"" +
          this.mode +
          '"',
      );
    }
  ...
  } else if (this.mode === "short_blackout") {
    this.profile = SHORTBLACKOUT_PROFILE;
  }
  this.baselineTime = options.baselineTime || this.profile.baselineTime;
  this.timePerDifficulty =
    options.timePerDifficulty || this.profile.timePerDifficulty;
  this.minimumSynergy =
    options.minimumSynergy || this.profile.defaultMinimumSynergy;
  this.maximumSynergy =
    options.maximumSynergy || this.profile.defaultMaximumSynergy;
  this.maximumIndividualSynergy =
    options.maximumIndividualSynergy ||
    this.profile.defaultMaximumIndividualSynergy;
  this.maximumSpill = options.maximumSpill || this.profile.defaultMaximumSpill;
  this.initialOffset =
    options.initialOffset || this.profile.defaultInitialOffset;
  this.maximumOffset =
    options.maximumOffset || this.profile.defaultMaximumOffset;
...
BingoGenerator.prototype.makeCard = function () {
  ...
  for (var i = 1; i <= 25; i++) {
    ...
    if (result.goal) {
      ...
      this.bingoBoard[nextPosition].name =
        result.goal[this.language] || result.goal.name;
...
BingoGenerator.prototype.chooseGoalForPosition = function (position) {
  ...
  for (
    ...
    for (var j = 0; j < goalsAtTime.length; j++) {
      ...
      if (
        this.maximumSynergy >= synergies.maxSynergy &&
        synergies.minSynergy >= this.minimumSynergy
      ) {
...
BingoGenerator.prototype.getEffectiveTypeSynergiesForRow = function (row) {
  var synergiesForSquares = this.calculateSynergiesForSquares(
    this.getOtherSquares(row),
  );
  var effectiveTypeSynergies = this.calculateEffectiveTypeSynergies(
    this.calculateCombinedTypeSynergies(synergiesForSquares),
  );
...
BingoGenerator.prototype.mergeTypeSynergies = function (
  typeSynergies,
  newTypeSynergies,
) {
...
BingoGenerator.prototype.calculateCombinedTypeSynergies = function (
  synergiesForSquares,
) {
  ...
  for (var type in typeSynergies) {
    if (type in subtypeSynergies) {
      combinedTypeSynergies[type] = typeSynergies[type].concat(
...
BingoGenerator.prototype.filterRowtypeSynergies = function (
  synergiesForSquares,
) {
  var rowtypeSynergies = {};
...
BingoGenerator.prototype.calculateEffectiveTypeSynergies = function (
  typeSynergies,
) {
...
BingoGenerator.prototype.filterSynergyValuesForType = function (
  type,
  synergies,
) {
...
BingoGenerator.prototype.calculateEffectiveSynergyForSquares = function (
  synergiesForSquares,
) {
  var typeSynergies = this.calculateCombinedTypeSynergies(synergiesForSquares);
  var rowtypeSynergies = this.filterRowtypeSynergies(synergiesForSquares);
  var effectiveTypeSynergies =
    this.calculateEffectiveTypeSynergies(typeSynergies);
  var rowSynergy = 0;
