...
isATypeAlreadyUsed = function (goal, typesList) {
  // takes a goal and returns if one of his types is already in usedTypes
...
bingoGenerator = function (bingoList, opts) {
  ...
  for (var i = 1; i <= 25; i++) {
    if (easyTiles.indexOf(i) > -1) {
      ...
      while (isATypeAlreadyUsed(bingoList[1][randIndex], usedTypes)) {
        //reroll until no type is in common
        randIndex = Math.floor(Math.random() * bingoList[1].length);
      }
      ...
      while (isATypeAlreadyUsed(bingoList[2][randIndex], usedTypes)) {
        //reroll until no type is in common
        randIndex = Math.floor(Math.random() * bingoList[2].length);
      }
      for (
        var le = 0;
        le < (bingoList[2][randIndex].types || []).length;
        le++
      ) {
      ...
      while (isATypeAlreadyUsed(bingoList[3][randIndex], usedTypes)) {
        //reroll until no type is in common
        randIndex = Math.floor(Math.random() * bingoList[3].length);
      }
      for (
        var la = 0;
        la < (bingoList[3][randIndex].types || []).length;
        la++
      ) {
      ...
      while (isATypeAlreadyUsed(bingoList[4][randIndex], usedTypes)) {
        //reroll until no type is in common
        randIndex = Math.floor(Math.random() * bingoList[4].length);
      }
      for (
        var lo = 0;
        lo < (bingoList[4][randIndex].types || []).length;
        lo++
      ) {
