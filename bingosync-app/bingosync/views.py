...
from bingosync.forms import GoalListConverterForm, JoinRoomForm, RoomForm
from bingosync.generators import GeneratorException, InvalidBoardException
...
from bingosync.models.game_type import ALL_VARIANTS, GameType
from bingosync.models.misc import SiteNotice
from bingosync.models.rooms import Game, LockoutMode, Player, Room
from bingosync.publish import (
    publish_chat_event,
    publish_color_event,
    publish_connection_event,
    publish_goal_event,
    publish_new_card_event,
    publish_revealed_event,
)
from bingosync.settings import SOCKETS_PUBLISH_URL, SOCKETS_URL
...
def rooms(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.warning(
                "RoomForm errors: %r, custom_json was: %r",
                form.errors,
...
def new_card(request):
    data = parse_body_json_or_400(
        request, required_keys=["room", "lockout_mode", "hide_card"]
    )
...
def goal_selected(request):
    data = parse_body_json_or_400(
        request, required_keys=["room", "slot", "color", "remove_color"]
    )
...
def chat_message(request):
    ...
    chat_event = ChatEvent(
        player=player, player_color_value=player.color.value, body=text
    )
    chat_event.save()
...
def join_room_api(request):
    ...
    if join_form.is_valid():
        ...
        return HttpResponse(
            join_form.errors.as_json(), content_type="application/json", status=400
        )
...
def _clear_session_player(session, room):
    ...
    try:
        ...
        logger.warning("Attempted to double-disconnect from room: %r", room)
    ...
    for key in required_keys:
        if key not in data:
            raise InvalidRequestJsonError(
                "Request is missing required key: '" + str(key) + "'"
            )
        elif data[key] == "":
            raise InvalidRequestJsonError(
                "Request has empty string for required key: '" + str(key) + "'"
            )
