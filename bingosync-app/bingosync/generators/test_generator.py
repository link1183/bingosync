        ...
        for game_type in TEST_TYPES:
            for seed in TEST_SEEDS:
                ...
                for goal in raw_golden_data:
                    self.assertNotIn(
                        '"name":', goal["name"], str(game_type) + ", " + str(seed)
                    )
                golden_data = freeze_dict(raw_golden_data)
                if (
                    golden_data in data_map
                    and game_type not in DUPLICATE_GUARD_BLACKLIST
                ):
                    original_type, original_seed = data_map[golden_data]
                    self.fail(
                        "got duplicate card from ({}, {}), original was ({}, {})".format(
                            game_type.name, seed, original_type.name, original_seed
                        )
                    )
                else:
...
class TimeoutTestCase(test.TestCase):
    ...
    def test_eval_timeout(self):
        try:
            GameType.celeste.generator_instance().eval(
...
for gt in TEST_TYPES:
    ...
    parametrize_test(
        test_class, "correctness_{.name}_{}", test_card_correctness, [[gt], TEST_SEEDS]
    )
