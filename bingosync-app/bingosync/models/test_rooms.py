...
from bingosync.models import ConnectionEvent, Game, GameType, LockoutMode, Player, Room
...
class RoomTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.previous_game = Game.from_board(
            25 * [{"name": "foo"}],
            room=self.room,
            game_type_value=TEST_GAME_TYPE.value,
            lockout_mode_value=LockoutMode.non_lockout.value,
            seed="1234",
        )

        self.current_game = Game.from_board(
            25 * [{"name": "foo"}],
            room=self.room,
            game_type_value=TEST_GAME_TYPE.value,
            lockout_mode_value=LockoutMode.non_lockout.value,
    ...
    def test_get_nonexist_room(self):
        self.assertRaises(
            Room.DoesNotExist, Room.get_for_encoded_uuid, "abcdefghijklmnopqrstuv"
        )
    ...
    def test_get_listed_rooms(self):
        ...
        self.assertEqual(
            {p.name for p in rooms[0].connected_players},
            {"the creator", "connected 1", "connected 2"},
        )
